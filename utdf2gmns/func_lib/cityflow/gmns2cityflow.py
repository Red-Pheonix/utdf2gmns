...
def generate_roads(network_nodes: dict, network_links: dict, network_unit: str):
    # manage unit conversions
    unit_speed, unit_distance, cvt_unit_speed, cvt_unit_distance = conversion_map(network_unit)

    roads = []
    for to_node_id, direction_links in network_links.items():
        for direction in direction_links:
            link = direction_links[direction]
            from_node_id = link.get("Up ID")

            from_node = network_nodes[from_node_id]
            to_node = network_nodes[to_node_id]
...
def generate_road_links(network_lanes, roads_map):
    node_to_road_links_map = {}
    for node_id, network_lane in network_lanes.items():
...
def generate_traffic_light_infos(utdf_dict, node_to_road_links_map):
    traffic_light_nodes = list(set(utdf_dict.get("Timeplans")["INTID"].tolist()))
    traffic_light_infos = {}
    for traffic_light_node in traffic_light_nodes:

        signal_plan = parse_signal_control(
            df_phase=utdf_dict.get("Phases"),
...
def generate_intersections(network_nodes, roads, node_to_road_links_map, traffic_phase_infos):
...
def generate_cityflow_net(utdf_dict: dict, network_unit: str):
    ...
    # prepare roads
    roads = generate_roads(network_nodes, network_links, network_unit)
    ...
    node_to_road_links_map = generate_road_links(network_lanes, roads_map)

    # prepare traffic lights
    traffic_phase_infos = generate_traffic_light_infos(utdf_dict, node_to_road_links_map)

    # prepare intersections
    intersections = generate_intersections(
        network_nodes, roads, node_to_road_links_map, traffic_phase_infos
    )
...
def generate_cityflow_flow(utdf_dict: dict, start_time: int, end_time: int):
    ...
    flow_items = []
    flow_ids = set()
    for inter_node_id, direction_lanes in network_lanes.items():
